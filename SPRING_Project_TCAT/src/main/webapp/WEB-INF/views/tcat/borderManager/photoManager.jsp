...
<title>사진관리 게시판</title>
<script type="text/javascript">
$("#photoManagerNomo").click(function(){
	...
	$("input[name=noMo_title]").focus();
		...
		formData.append("HnoMoboard_Image",$("input[name=HnoMoboard_Image]")[0].files[0]);
		$.ajax({
			url : 'photoManagerNomo',
			data : formData,
			processData : false,
			contentType : false,
			type : 'POST',
    ...
    <div class="col-md-10">
    	 <c:forEach var="vo" items="${dtos}" > 
			...
			<div>				
				...
				<input type="button" class="btn btn-danger" value="삭제" onclick="load('photoManagerDelete?notice_num=${vo.notice_num}&category=${vo.notice_div}');" >
			</div>
			</form>
    	...
    	<div class="col-md-1"></div>	
			...
			<div class="col-sm-10 col-md-10">
			 <h4><b>공연 사진게시판 수정</b></h4>
				<hr>
				<div class="thumbnail c" >
				<form action="" name="photoModifyManagerForm" onsubmit="return false">
				<div class="disInline ">
				<input type="hidden" name="monotice_num" value="${vo.notice_num}">
						<table class="borderCollSe borderSpace20 w100p" >
							<tr>						
								...
								<td><input type="file" name="HnoMoboard_Image" style="width: 100%"></td>
					...
					<hr>
					<input type="submit" class="btn btn-info" id="photoManagerNomo" value="수정하기" >
				</form>
				</div><!-- outline -->
			</div><!-- md10 -->
		 <div class="col-md-1"></div>	 
		 ...
		 <div class="col-md-5 c">
		  <table class="c" style="width: 1000px" align="center">
						...
						<c:if test="${startPage > pageBlock}">
							<a onclick="load('photoManager');">[◀◀]</a>
							<a onclick="load('photoManager?pageNum=${startPage - pageBlock}');">[◀]</a>
						</c:if>
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							...
							<c:if test="${i != currentPage}">
								<a onclick="load('photoManager?pageNum=${i}');">[${i}]</a>
							</c:if>
						</c:forEach>
						...
						<c:if test="${pageCount > endPage}">
							<a onclick="load('photoManager?pageNum=${startPage + pageBlock}');">[▶]</a>
							<a onclick="load('photoManager?pageNum=${pageCount}');">[▶▶]</a>
						</c:if>
					</c:if></th>
					</tr>
				</table>
