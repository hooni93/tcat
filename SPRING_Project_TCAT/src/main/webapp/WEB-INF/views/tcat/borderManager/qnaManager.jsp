				...
				<c:if test="${cnt > 0}">
					<c:forEach var="dto" items="${dtos}">
						<tr>
							...
							<td><a onclick="load('qnaManagerContent?service_num=${dto.service_num}');">${dto.service_title}</a></td>
							<td>${dto.member_id}</td>
							<td>
					...
					<table
						...
						<c:if test="${cnt > 0}">
							...
							<c:if test="${startPage > pageBlock}">
								<a onclick="load('qnaManager?cDev=${cDev}&keyword=${keyword}');">[◀◀]</a>
								<a href="load('qnaManager?pageNum=${startPage - pageBlock}&cDev=${cDev}&keyword=${keyword}');">[◀]</a>
							</c:if>
							...
							<c:forEach var="i" begin="${startPage}" end="${endPage}">
								...
								<c:if test="${i != currentPage}">
									<a href="load('qnaManager?pageNum=${i}&cDev=${cDev}&keyword=${keyword}');">[${i}]</a>
								</c:if>
							</c:forEach>
							...
							<c:if test="${pageCount > endPage}">
								<a href="load('qnaManager?pageNum=${startPage + pageBlock}&cDev=${cDev}&keyword=${keyword}');">[▶]</a>
								<a href="load('qnaManager?pageNum=${pageCount}&cDev=${cDev}&keyword=${keyword}');">[▶▶]</a>
							</c:if>
						</c:if>
					</table>
				</div>
