									...
									<tr>
										...
										<td><c:if test="${vo.size()!=null}">
												<button type="button" class="btn btn-primary"
													onclick="return hotDelete('${vo.size()}','${v.per_id}','hotDance')">삭제</button>
											</c:if></td>
					...
					<div>
								...
								<c:forEach var="full" items="${full}">
									<tr>
										...
										<td><c:if test="${vo.size()==null}">
												...
												<button type="button" class="btn btn-primary"
													onclick="return hotUpdate('${vo.size()}','${full.per_id}','hotDance')">추가</button>
											</c:if></td>
									</tr>
								</c:forEach>
							</c:if>
							...
							<tr>
								<th align="center"><c:if test="${cnt > 0}">
										...
										<c:if test="${startPage > pageBlock}">
											<a
												onclick="load('hotDance?pageNum=${startPage - pageBlock}');">[◀]</a>
										</c:if>
										...
										<c:forEach var="i" begin="${startPage}" end="${endPage}">
											...
											<c:if test="${i != currentPage}">
												<a
													onclick="load('hotDance?pageNum=${i}');">[${i}]</a>
											</c:if>
										</c:forEach>
										<!-- 다음[▶] / 끝[▶▶] -->
										<c:if test="${pageCount > endPage}">
											<a
												onclick="load('hotDance?pageNum=${startPage + pageBlock}');">[▶]</a>
											<a
												onclick="load('hotDance?pageNum=${pageCount}');">[▶▶]</a>
										</c:if>
									</c:if></th>
							</tr>
						</table>
						...
						<form class="navbar-form navbar-right" role="search"
							...
							<button type="button" class="btn btn-default"
								onclick="ajaxSubmit('hotDance')">검색</button>
