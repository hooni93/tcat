		...
		<div class="col-md-10">
			<div class="row">
				...
				<div class="col-md-10">
									...
									<tr>
										...
										<td><c:if test="${vo.size()!=null}">
												<button type="button" class="btn btn-primary"
													onclick="return hotDelete('${vo.size()}','${v.per_id}','hotConcert')">삭제</button>
											</c:if></td>
					...
					<br> <br> <br>
					<h4 class="trgothic">콘서트 전체리스트</h4>
					<div>
						<hr>
						<!-- 뮤지컬전체리스트 -->
						<table
								...
								<c:forEach var="full" items="${full}">
									<tr>
										...
										<td><c:if test="${vo.size()==null}">
												...
												<button type="button" class="btn btn-primary"
													onclick="return hotUpdate('${vo.size()}','${full.per_id}','hotConcert')">추가</button>
											</c:if></td>
									</tr>
								</c:forEach>
							</c:if>
							...
							<tr>
								<th align="center"><c:if test="${cnt > 0}">
										...
										<c:if test="${startPage > pageBlock}">
											<a
												onclick="load('hotConcert?pageNum=${startPage - pageBlock}');">[◀]</a>
										</c:if>
										...
										<c:forEach var="i" begin="${startPage}" end="${endPage}">
											...
											<c:if test="${i != currentPage}">
												<a
													onclick="load('hotConcert?pageNum=${i}');">[${i}]</a>
											</c:if>
										</c:forEach>
										...
										<c:if test="${pageCount > endPage}">
											<a
												onclick="load('hotConcert?pageNum=${startPage + pageBlock}');">[▶]</a>
											<a
												onclick="load('hotConcert?pageNum=${pageCount}');">[▶▶]</a>
										</c:if>
									</c:if></th>
							</tr>
						</table>
						...
						<form class="navbar-form navbar-right" role="search"
							...
							<button type="button" class="btn btn-default"
								onclick="ajaxSubmit('hotConcert')">검색</button>
