				...
				<div class="col-md-10">
					...
					<div>
						<table
							...
							<c:if test="${cnt==0}">
								<tr>
									<td colspan="7" align="center">환불요청이 없습니다.</td>
								</tr>
							</c:if>
						</table>
							...
							<tr>
								<th align="center"><c:if test="${cnt > 0}">
										...
										<c:if test="${startPage > pageBlock}">
											<a
												onclick="load('productRefund?pageNum=${startPage - pageBlock}');">[◀]</a>
										</c:if>
										...
										<c:forEach var="i" begin="${startPage}" end="${endPage}">
											...
											<c:if test="${i != currentPage}">
												<a
													onclick="load('productRefund?pageNum=${i}');">[${i}]</a>
											</c:if>
										</c:forEach>
										<!-- 다음[▶] / 끝[▶▶] -->
										<c:if test="${pageCount > endPage}">
											<a
												onclick="load('productRefund?pageNum=${startPage + pageBlock}');">[▶]</a>
											<a
												onclick="load('productRefund?pageNum=${pageCount}');">[▶▶]</a>
										</c:if>
									</c:if></th>
							</tr>
						</table>
