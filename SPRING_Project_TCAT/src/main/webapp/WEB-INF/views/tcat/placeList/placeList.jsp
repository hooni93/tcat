			...
			<div class="col-sm-12 c">
				...
				<ul class="pagination">
					<c:if test="${currentPage!=1}">
						<li><a
							onclick="place('placeList?place=${place}&pageNum=1','${place}');">《</a></li>
						<li><a
							onclick="place('placeList?place=${place}&pageNum=${currentPage-1}','${place}');">〈</a></li>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<li id="${i }"><a
							onclick="place('placeList?place=${place}&pageNum=${i}','${place}');">${i }</a></li>
					</c:forEach>
					<c:if test="${currentPage!=pageCnt}">
						<li><a
							onclick="place('placeList?place=${place}&pageNum=${currentPage+1}','${place}');">〉</a></li>
						<li><a
							onclick="place('placeList?place=${place}&pageNum=${pageCnt}','${place}');">》</a></li>
					</c:if>
				</ul>
			</div>
</body>
