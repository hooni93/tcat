...
<script type="text/javascript">
	$(function(){
		$("#write3").click(function(){
			...
			formData.append("service_num",$("input[name=service_num]").val());
			$.ajax({
				url : 'qnaWriteUpdatePro',
				data : formData,
				processData : false,
				contentType : false,
				type : 'POST',
...
<body>
	...
	<div class="col-md-10">
		<h1><center>Q&A게시글 수정페이지 </center></h1>
		<hr><br>
		<table class="table table-hover table-bordered table-condensed c fs12">
			<tr>
				...
				<td>
					...
					<%int service_num = Integer.parseInt(request.getParameter("service_num")); %>
					<input type="hidden" value="${service_num}" name="service_num">
				</td>
			</tr>
		...
		<div>
			<center>
				<!-- <button type="button" id="write" style="border: 2px solid #cccccc;">작성</button> -->
			 	...
			 	<button type="button" id="write3" class="btn btn-primary btn-sm btn btn-info">작성</button>
			 	<input type="reset" value="취소" 
			 	    class="btn btn-primary btn-sm btn btn-info">
			 	<input type="button" value="목록보기" onclick="loadBoard('qnaBoardList');"
			 		class="btn btn-primary btn-sm btn btn-info">
			 	<!-- <input type="button" value="수정하기" onclick="loadBoard('qnaBoardList');"
			 		class="btn btn-primary btn-sm btn btn-info"> -->	
			</center>
		</div>
		<br><br>
