			...
			<div class="sidebar-nav disInline w100p floatR">
				<div class="well borderMenu disInline w100p"
					...
					<ul class="nav nav-list  w100p ">
						<li><a class="list_side"><b>클래식</b></a>
							<hr class="w90p" style="margin: 0 auto;"></li>
						<li><a
							onclick="load('classicList?sDev=클래식&order=startDate');"
							class="list_side">클래식</a></li>
						<li><a
							onclick="load('classicList?sDev=국악&order=startDate');"
							class="list_side">국악</a></li>
					</ul>
				</div>
			</div>
		</div>
			...
			<div class="col-sm-12 c">
				...
				<div class="col-sm-5">
					<a class="order" id="startDate"
						onclick="load('classicList?sDev=${sDev}&order=startDate');">등록순</a>
					<img src="${image}bar/bl_gray.gif"> <a class="order"
						id="endDate"
						onclick="load('classicList?sDev=${sDev}&order=endDate');">종료임박순</a>
					<img src="${image}bar/bl_gray.gif"> <a class="order"
						id="perf_title"
						onclick="load('classicList?sDev=${sDev}&order=perf_title');">상품명순</a>
				</div>
			...
			<div class="col-sm-12 c">
				...
				<ul class="pagination">
					<c:if test="${currentPage!=1}">
						<li><a
							onclick="load('classicList?sDev=${sDev }&order=${order}&pageNum=1');">《</a></li>
						<li><a
							onclick="load('classicList?sDev=${sDev }&order=${order}&pageNum=${currentPage-1}');">〈</a></li>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<li id="${i }"><a
							onclick="load('classicList?sDev=${sDev }&order=${order}&pageNum=${i}');">${i }</a></li>
					</c:forEach>
					<c:if test="${currentPage!=pageCnt}">
						<li><a
							onclick="load('classicList?sDev=${sDev }&order=${order}&pageNum=${currentPage+1}');">〉</a></li>
						<li><a
							onclick="load('classicList?sDev=${sDev }&order=${order}&pageNum=${pageCnt}');">》</a></li>
					</c:if>
				</ul>
