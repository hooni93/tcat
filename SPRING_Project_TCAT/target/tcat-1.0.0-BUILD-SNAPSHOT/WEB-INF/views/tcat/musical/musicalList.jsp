	...
	<div class="col-md-10">
			...
			<div class="sidebar-nav disInline w100p floatR">
				<div class="well borderMenu disInline w100p"
					...
					<ul class="nav nav-list  w100p ">
						...
						<li><a
							onclick="load('musicalList?sDev=오리지널&order=startDate');"
							class="list_side">오리지널</a></li>
						<li><a
							onclick="load('musicalList?sDev=라이선스&order=startDate');"
							class="list_side">라이선스</a></li>
						<li><a onclick="load('musicalList?sDev=창작&order=startDate');"
							class="list_side">창작뮤지컬</a></li>
						<li><a
							onclick="load('musicalList?sDev=넌버벌&order=startDate');"
							class="list_side">넌버벌</a></li>
					</ul>
				</div>
			</div>
		</div>
		...
		<div class="col-md-9">
			<div class="col-sm-12 c">
				<div class="col-sm-6 fs11 pt3">현재 예매 가능한 ${sDev } 뮤지컬이 총
					${productCnt}개 있습니다.</div>
				<div class="col-sm-5">
					<a class="order" id="startDate"
						onclick="load('musicalList?sDev=${sDev}&order=startDate');">등록순</a>
					<img src="${image}bar/bl_gray.gif"> <a class="order"
						id="endDate"
						onclick="load('musicalList?sDev=${sDev}&order=endDate');">종료임박순</a>
					<img src="${image}bar/bl_gray.gif"> <a class="order"
						id="perf_title"
						onclick="load('musicalList?sDev=${sDev}&order=perf_title');">상품명순</a>
				</div>
				...
				<div class="col-sm-12 c" style="border-bottom: 1px solid #d0d0d0">
					<div class="col-sm-8 tm">
						<div class="col-sm-3 p5">
							<a onclick="contentPage(${perf.per_id})"> <img class="img-responsive"
								src="${image}performance/${perf.perf_Image }" width="100%"
								height="80px">
							</a>
						</div>
						<div class="col-sm-9 pt18">
							<b class="floatL"> <a onclick="contentPage(${perf.per_id})"
								style="color: black"> ${perf.perf_title } </a>
							</b>
						</div>
					</div>
			...
			<div class="col-sm-12 c">
				...
				<ul class="pagination">
					<c:if test="${currentPage!=1}">
						<li><a
							onclick="load('musicalList?sDev=${sDev }&order=${order}&pageNum=1');">《</a></li>
						<li><a
							onclick="load('musicalList?sDev=${sDev }&order=${order}&pageNum=${currentPage-1}');">〈</a></li>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<li id="${i }"><a
							onclick="load('musicalList?sDev=${sDev }&order=${order}&pageNum=${i}');">${i }</a></li>
					</c:forEach>
					<c:if test="${currentPage!=pageCnt}">
						<li><a
							onclick="load('musicalList?sDev=${sDev }&order=${order}&pageNum=${currentPage+1}');">〉</a></li>
						<li><a
							onclick="load('musicalList?sDev=${sDev }&order=${order}&pageNum=${pageCnt}');">》</a></li>
					</c:if>
				</ul>
