						...
						<table
							...
							<c:if test="${cnt > 0}">
								...
								<c:if test="${startPage > pageBlock}">
									<a onclick="categoryload('stockDelete_musical');">[◀◀]</a>
									<a onclick="categoryload0('stockDelete_musical','${startPage - pageBlock}');">[◀]</a>
								</c:if>
								...
								<c:forEach var="i" begin="${startPage}" end="${endPage}">
									...
									<c:if test="${i != currentPage}">
										<a onclick="stockDelete_musical?pageNum=${i}">[${i}]</a>
									</c:if>
								</c:forEach>
								...
								<c:if test="${pageCount > endPage}">
									<a onclick="categoryload0('stockDelete_musical','${startPage + pageBlock}');">[▶]</a>
									<a onclick="categoryload0('stockDelete_musical','${pageCount}');">[▶▶]</a>
								</c:if>
							</c:if>
						</table>
					</div>
